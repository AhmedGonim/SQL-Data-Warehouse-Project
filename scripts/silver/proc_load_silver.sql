...
BEGIN
		...
		*,
		...
		END AS sls_due_dt,
		CASE WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quantity * ABS(sls_price)   --ABS >> تحول الرقم السلبي الي موجب
		...
		CASE WHEN sls_price IS NULL OR sls_price <= 0 
			 ...
			 ELSE sls_price
		END AS sls_price
		FROM bronze.crm_sales_details;SET @end_time = GETDATE();
		PRINT '>> Load Duration: ' + CAST (DATEDIFF(second , @start_time, @end_time ) AS VARCHAR ) + ' second'
		PRINT '-----------------------------'
	...
	BEGIN CATCH
		PRINT '===================================================';
		PRINT  'ERROR OUCCRED DURING LOADING Silver LAYER';
